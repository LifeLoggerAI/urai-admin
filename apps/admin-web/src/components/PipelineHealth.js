...
exports.default = PipelineHealth;
var healthMetrics = [
...
function PipelineHealth() {
    return (<div className="mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {healthMetrics.map(function (metric) { return (<div key={metric.name} className="p-4 rounded-lg bg-zinc-100 dark:bg-zinc-800">
                <div className="text-sm font-medium text-zinc-500 dark:text-zinc-400">{metric.name}</div>
                <div className={"text-2xl font-semibold ".concat(metric.status === 'Healthy' ? 'text-green-500' :
                metric.status === 'Warning' ? 'text-yellow-500' :
                    'text-red-500')}>{metric.status}</div>
