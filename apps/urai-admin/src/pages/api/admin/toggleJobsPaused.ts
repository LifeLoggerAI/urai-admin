  ...
  try {
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      transaction.set(auditRef, {
        ...
        action: 'system.jobsPaused.update',
        target: { type: 'systemConfig', id: 'runtime' },
        metadata: { before: before?.jobsPaused ?? null, after: jobsPaused },
        createdAt: new Date(),
      });
    });

    return res.status(200).json({ success: true, jobsPaused });
  } catch (error) {
    console.error('Failed to update jobs paused:', error);
