...
export default function AdminEntryPage() {
  return (
    <main className="urai-shell text-white">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:rounded-lg focus:bg-white focus:px-4 focus:py-3 focus:text-slate-950">
        Skip to content
      </a>
      ...
      <div id="main-content">
        <section className="relative z-10 mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:py-24" aria-labelledby="admin-title">
          <div>
            ...
            <h1 id="admin-title" className="max-w-3xl text-5xl font-semibold leading-[0.98] tracking-[-0.055em] md:text-7xl">
              Operate the wheel without losing the truth.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
              URAI Admin is the protected control plane for systems, releases, evidence, workflows, providers, policy, incidents, domains, cost, audit, and governance.
            </p>
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-6 py-14" aria-labelledby="control-plane-title">
          <div className="mb-8 max-w-3xl">
            <div className="neon-pill mb-4 inline-flex rounded-full px-3 py-1 text-sm">Control plane</div>
            <h2 id="control-plane-title" className="text-3xl font-semibold tracking-[-0.035em] md:text-4xl">The cockpit is organized around evidence, not spectacle.</h2>
            <p className="mt-4 text-slate-300">Every area answers a different operational question while preserving the system boundary that owns the underlying truth.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {controlPlaneAreas.map((area) => (
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-6 py-14" aria-labelledby="truth-state-title">
          <div className="hero-glass rounded-3xl p-7 md:p-10">
            <div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr]">
              <div>
                <div className="neon-pill mb-4 inline-flex rounded-full px-3 py-1 text-sm">Truth states</div>
                <h2 id="truth-state-title" className="text-3xl font-semibold tracking-[-0.035em]">Missing data is a state, not an invitation to invent a metric.</h2>
                <p className="mt-4 text-sm leading-6 text-slate-300">When a provider, registry, release, or evidence source is unavailable, Admin names the gap and protects the decision boundary.</p>
              </div>
              <dl className="grid gap-3 sm:grid-cols-2">
                {truthStates.map(([state, meaning]) => (
                  <div key={state} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                    <dt className="text-xs font-bold tracking-[0.12em] text-cyan-100">{state}</dt>
                    <dd className="mt-2 text-sm leading-6 text-slate-300">{meaning}</dd>
                  </div>
                ))}
              </dl>
            </div>
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-6 py-14" aria-labelledby="admin-principles-title">
          <div className="border-t border-white/10 pt-10">
            ...
            <ul className="mt-6 grid gap-3 text-sm text-slate-200 md:grid-cols-2">
              {operatingPrinciples.map((principle) => (
                <li key={principle} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">{principle}</li>
              ))}
            </ul>
          </div>
        </section>
